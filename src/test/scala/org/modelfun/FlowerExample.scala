...
object FlowerExample extends StandardDisplay {
  def setup() {
    ...
    val flower = OnCircle(spoke=petal, center=core, spokes=17, radius = 3, upAngle = 0.1 turns)
    ...
    val leaves = AlongPath(leaf, path=stemPath, minDistance=5, density=Gauss, align=AlignAround(angle=0.25 turns, stepAround = 0.3turns))
